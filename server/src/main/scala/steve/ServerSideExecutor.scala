  ...
  def instance[F[_]: Interpreter: MonadThrow]: Executor[F] =
    new Executor[F] {
      ...
      private val resolveCommand: Build.Command => ResolvedBuild.Command = {
        case Build.Command.Upsert(key, value) =>
          ResolvedBuild.Command.Upsert(key, value)
        case Build.Command.Delete(key) =>
          ResolvedBuild.Command.Delete(key)
      }
      private def resolve(build: Build): F[ResolvedBuild] =
        (build == Build.empty)
        .guard[Option]
        .as(emptyState)
      ...
      def build(build: Build): F[Hash] =
        resolve(build).flatMap(Interpreter[F].interpret).flatMap {
